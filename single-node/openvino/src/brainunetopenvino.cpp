...
//#define M_IE_PLUGIN_PATH "/opt/intel/computer_vision_sdk_2018.3.343/deployment_tools/inference_engine/lib/ubuntu_16.04/intel64"
#define M_IE_PLUGIN_PATH "/opt/intel/computer_vision_sdk/deployment_tools/inference_engine/lib/ubuntu_16.04/intel64"

using namespace InferenceEngine;

void BrainUnetOpenVino::loadNumpyData(cnpy::NpyArray &arr, cnpy::NpyArray &arr_msks)
{
        //************************* reading nmpy images****************************/
...
void BrainUnetOpenVino::makeInference(int img_num, InferenceEngine::TargetDevice targetDevice, cnpy::NpyArray &arr, cnpy::NpyArray &arr_msks )
{
    //************************* reading nmpy images****************************/
   ...
   //make sure the loaded data matches the saved data
    ...
    std::cout<<" H ="<<arr.shape[1]<<" W ="<< arr.shape[2]<<" C ="<< arr.shape[3]<<" N ="<< arr.shape[0]<<std::endl;
  //Store the imgs for validation from numpy array into 1-D array
   ...
   //***********Initialize Engin plugin, choose plugin type and set precission****************************//
    InferenceEnginePluginPtr engine_ptr;
    engine_ptr = PluginDispatcher( { "/opt/intel/computer_vision_sdk/deployment_tools/inference_engine/lib/ubuntu_16.04/intel64","" } ).getSuitablePlugin(targetDevice);
    ...
    std::string network_model_path = "../models/" + target_precision + "/saved_model.xml";
    std::string network_weights_path = "../models/" + target_precision + "/saved_model.bin";
   ...
   if (input_info.size() != 1) throw std::logic_error("Sample supports topologies only with 1 input");
   ...
   std::cout << "** Inference request has been created." << std::endl;
    ...
    auto blob_data = inputBlob->buffer().as<PrecisionTrait<Precision::FP32>::value_type*>();
    int c1;
    for (c1 = 0; c1 < Nx*Ny*Nz; c1++)  //currently hardcoded for image size 144 X 144
       {
   ...
   // --------------------------- 8. Process output -------------------------------------------------------
    ...
    const auto predicted_output_msk = output_blob->buffer().as<PrecisionTrait<Precision::FP32>::value_type*>();
    auto finish = std::chrono::high_resolution_clock::now();
    float inf_time = (std::chrono::duration_cast<std::chrono::milliseconds>(finish - start).count());
    std::cout<<"Inference Done; inference time  "<<inf_time<<"\n";


   //-----------------------------9. Compute some metrics from segmentation output and Display results----------------------------------------------

    //read corresponding GT and actual brain image from the image directories change paths
    ...
    std::cout<<"gt cnt  "<<gt_cnt<<" predicted_cnt "<<predicted_cnt<<" total_union  "<<total_union<<" intersection "<<intersection<<"\n";
    if(predicted_cnt==0){
