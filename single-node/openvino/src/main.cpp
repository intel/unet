...
void printUsage(const string &app_name = string()) {
  ...
  if (!app_name.empty())
    cout << app_name << endl;
  cout << "     [-h] [--h] [-help] [--help] : display this usage page. No "
          "other commands accepted."
       << endl;
  cout << "     [-p MYRIAD | myriad | CPU | cpu] : select plugin (defaults is "
          "CPU)"
       << endl;
  cout << "     [-f filename] : specify the image number to be detected and "
          "recognized\n"
       << endl;
}
